...
import { Supplier, Product, PurchaseOrder, OrderItem, User } from '../../types';
import { Plus, Trash2 } from 'lucide-react';

type NewPurchaseOrderData = Omit<PurchaseOrder, 'id' | 'createdAt'>;
...
interface NewPurchaseOrderModalProps {
  ...
  products: Product[];
  users: User[];
  onAddOrder: (order: NewPurchaseOrderData) => void;
  showToast: (message: string, type?: 'success' | 'error') => void;
}

const NewPurchaseOrderModal: React.FC<NewPurchaseOrderModalProps> = ({ isOpen, onClose, suppliers, products, users, onAddOrder, showToast }) => {
    ...
    }, [products, suppliers, users, isOpen]);
    ...
    const handleSubmit = (e: React.FormEvent) => {
        ...
        if (!supplier || currentItems.length === 0 || !assignedTo) {
            showToast('Selecione um fornecedor e adicione pelo menos um produto.', 'error');
            return;
        }
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
                ...
                <div>
                    ...
                    <div className="mt-1">
                      ...
                      {selectedProduct && (
                        <p className="text-right text-sm text-neutral-medium dark:text-gray-400 mt-1 pr-12">
                          Custo Est.: {selectedProduct.price.toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' })}
                        </p>
                      )}
                    </div>
                </div>
                ...
                <div className="pt-4 border-t border-gray-200 dark:border-gray-700">
                     <div className="mb-4">
                        <label htmlFor="assignedTo" className="block text-sm font-medium text-gray-700 dark:text-gray-300">Responsável</label>
                        <select id="assignedTo" value={assignedToId} onChange={e => setAssignedToId(e.target.value)} className="mt-1 block w-full pl-3 pr-10 py-2 text-base border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white focus:outline-none focus:ring-primary/50 focus:border-primary/50 sm:text-sm rounded-md">
                            {users.map(u => <option key={u.id} value={u.id}>{u.name}</option>)}
                        </select>
                    </div>
                    <div className="flex justify-between items-center text-right">
                        <span className="text-lg font-semibold text-neutral-dark dark:text-white">Total Estimado:</span>
                        <span className="text-xl font-bold text-primary">{total.toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' })}</span>
                    </div>
                </div>

                <div className="flex justify-end space-x-2 pt-4">
                    <button type="button" onClick={handleClose} className="py-2 px-4 bg-gray-200 dark:bg-gray-600 text-gray-800 dark:text-white rounded-lg hover:bg-gray-300 dark:hover:bg-gray-500">Cancelar</button>
                    <button type="submit" className="py-2 px-4 bg-action text-white font-bold rounded-lg hover:bg-action/90">Salvar Pedido</button>
                </div>
            </form>
        </Modal>
    );
